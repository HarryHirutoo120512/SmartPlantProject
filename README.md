...
| Backend API | `backend/` |
| Frontend (Node-RED) | Tự xây trên Node-RED — xem `docs/NODERED.md` |
...
2. Khởi động Node-RED:
   ...
   node-red
   ```
3. Mở trình duyệt: **http://localhost:1880**
4. Cài Dashboard (Manage palette → Install):
   - `node-red-dashboard` — gauge, chart, button, notification
5. Thiết kế flow theo hướng dẫn chi tiết tại **[docs/NODERED.md](docs/NODERED.md)**:
   - Đăng nhập lấy JWT token
   - Hiển thị cảm biến (gauge/text)
   - Nút điều khiển bơm/quạt
   - Biểu đồ lịch sử từ Cloud
   - Thông báo cảnh báo realtime
   - Form cấu hình ngưỡng

6. Dashboard UI truy cập tại: **http://localhost:1880/ui**
